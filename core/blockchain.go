...
var (
	...
	accountTrieUpdateTimer      = metrics.NewRegisteredTimer("chain/account/trie/updates", nil)
	accountTrieUpdateInnerTimer = metrics.NewRegisteredTimer("chain/account/trieinner/updates", nil)
	accountOriginUpdateTimer    = metrics.NewRegisteredTimer("chain/account/origin/updates", nil)
...
func (c *CacheConfig) triedbConfig(keepFunc pathdb.NotifyKeepFunc) *triedb.Config {
...
func (bc *BlockChain) insertChain(chain types.Blocks, setHead bool) (int, error) {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		accountTrieUpdateTimer.Update(statedb.AccountTrieUpdates)
		accountTrieUpdateInnerTimer.Update(statedb.AccountTrieUpdateInner)
		accountOriginUpdateTimer.Update(statedb.AccountOriginUpdates)
		storageUpdateTimer.Update(statedb.StorageUpdates) // Storage updates are complete(in validation)
		accountHashTimer.Update(statedb.AccountHashes)    // Account hashes are complete(in validation)
		storageHashTimer.Update(statedb.StorageHashes)    // Storage hashes are complete(in validation)
		txDAGGenerateTimer.Update(statedb.TxDAGGenerate)
		blockStateFinalizeInRootTimer.Update(statedb.FinalizeInRootTime)
		blockAccountRootTimer.Update(statedb.AccountRootTime)
		blockStateRootTimer.Update(statedb.StateRootTime)
